...
// These are created in USB0_DESCRIPTOR.c
...
extern SEGMENT_VARIABLE(INTERFACE_INTERRUPT_DESC, const Interface_Descriptor, SEG_CODE);
...
extern SEGMENT_VARIABLE(IN_EP3_DESC, const Endpoint_Descriptor, SEG_CODE);
extern SEGMENT_VARIABLE_SEGMENT_POINTER(STRING_DESC_TABLE[], U8, const SEG_CODE, const SEG_CODE);

// These are created in F3xx_USB0_InterruptServiceRoutine.c
...
SEGMENT_VARIABLE(ONES_PACKET[2], const U8, SEG_CODE) = {0x01, 0x00};
SEGMENT_VARIABLE(ZERO_PACKET[2], const U8, SEG_CODE) = {0x00, 0x00};
   ...
   {						
      ...
      case OUT_ENDPOINT:
         ...
         else
         {
            ...
            // Handle case if request is directed to IN_EP3
            else if (Setup.wIndex.U8[LSB] == IN_EP3)
            {
               // If endpoint is halted, return 0x01,0x00
...
void Get_Descriptor (void)
{
   ...
   switch (Setup.wValue.U8[MSB])
   {
      ...
      case DSC_INTERFACE:
         if (Setup.wValue.U8[LSB] == 0x00)
         {
            DataPtr = (U8*) &INTERFACE_INTERRUPT_DESC;
            DataSize = INTERFACE_INTERRUPT_DESC.bLength;
         }
         break;
      ...
      case DSC_ENDPOINT:
             ...
             (Setup.wValue.U8[LSB] == IN_EP3))
         {
            ...
            else if (Setup.wValue.U8[LSB] == OUT_EP1)
            {
            ...
            {
               DataPtr = (U8*) &IN_EP3_DESC;
               DataSize = IN_EP3_DESC.bLength;
            }
         }
         else
         {
...
void Set_Configuration (void)
{
   ...
   else
   {
      ...
      if (Setup.wValue.U8[LSB] > 0)
      {
         ...
         // set DIRSEL to indicate endpoint 3 is IN
         // set interrupt mode
         POLL_WRITE_BYTE (EINCSRH, EINCSRH_DIRSEL__IN |
                          EINCSRH_ISO__DISABLED);
         Handle_In3();
